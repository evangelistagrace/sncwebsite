...
if (!class_exists('Wp_Migration_S3')) {
    ...
    class Wp_Migration_S3
    {
		...
		$out['export_option'] = 's3bucket';
                ...
                if ( $connector ) {
                    try {
                        $storage_location_array = explode('/', $s3bucketname);
                        $storage_location = '';
        ...
        function authenticate_s3bucket()
        {
            ...
            try {
                ...
                Webtoffe_logger::error($exception->getMessage());
                wp_send_json_error(__('The AWS Access credentials you provided does not exist in our records', 'wp-migration-duplicator'));
            }
            $this->status = true;
...
?>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <span class="wt-cli-cloud-status wt-cli-cloud-status-enabled">
                                ...
                                <a  href="<?php echo admin_url('admin.php?page=wp-migration-duplicator-settings#wt-s3bucket')?>"><?php echo __('Settings','wp-migration-duplicator-pro')?></a>
                            </td>
                        </tr>
                        <tr>
                            <th>
                            ...
                            <td>
                                <select class="wt-migrator-select2-single wt-migrator-cloud-import-file wt-migrator-input wt-migrator-file"  data-hidden-name="wt_mgdb_s3bucket_file" style="width:100%">
                                    <option value="-1"><?php echo __('Please select your backup file','wp-migration-duplicator');?></option>
                                </select>
        ...
        {
        ?>
            <div class="wt-migrator-cloud-storage-info child-form-item child-wt_mgdb_export_option wt_mgdb_export_option_s3bucket" style="display:none">
                <?php
                $url = admin_url('admin.php?page=wp-migration-duplicator-settings#wt-s3bucket');
                ?>
                <span class="wt-migrator-authentication-error"><?php echo sprintf(wp_kses(__('Your account with Amazon S3 is not authenticated please click <a href="%s">here</a> to authenticate', 'wp-migration-duplicator'), array('a' => array('href' => array(), 'target' => array()))), esc_url($url)); ?></span>
                <table class="wf-form-table" style="max-width:650px;">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" class="wt-migrator-input wt-migrator-file" name="wt_mgdb_s3bucket_file_name" disabled placeholder="e.g. critical-backup" />
                            </td>
                        </tr>
                    </tbody>
                </table>
