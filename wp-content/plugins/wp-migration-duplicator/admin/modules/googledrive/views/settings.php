...
<div style = "display: flex">
	<img src="<?php echo plugins_url(basename(plugin_dir_path(WT_MGDP_PLUGIN_FILENAME))).'/admin/images/google-drive.svg'; ?>" style="max-width:20px;"/>&nbsp&nbsp<h3><?php _e('Google Drive','wp-migration-duplicator');?></h3> &nbsp&nbsp&nbsp&nbsp <?php if( $authenticated === true ):?> <h4 style="color: green;"><?php _e('Connected','wp-migration-duplicator');?> </h4> <?php endif; ?>
</div>
...
<form method="post"  action="<?php echo esc_url($_SERVER["REQUEST_URI"]);?>" id="wt_mgdp_googledrive">
	...
	<table class="form-table wf-form-table">
		...
		<tr>
			...
			<td>
                            <input type="password" name="wt_google_client_secret" value="<?php echo $client_secret; ?>" style="width:100%;border: 1px solid #ced4da;">
			</td>
			<td></td>
		</tr>
	</table>
