               ...
               $out['export_option'] = 'googledrive';
                if ( $this->client ) {
                    ...
                    if ($client->getAccessToken()) {
                        ...
                        $file->setDescription(__('backup using WP Migrator Duplicator Pro', 'wp-migration-duplicator'));
                        try {
...
?>
            ...
            <div class="wt-migrator-cloud-storage-info child-form-item child-wt_mgdb_import_option wt_mgdb_import_option_googledrive" style="display:none">
                ...
                <table class="wf-form-table wt_mgdp_import_options " style="max-width:650px;">
                    <tbody>
                        <tr>
                            <th><?php _e('Enable Google drive', 'wp-migration-duplicator') ?></th>
                            <td>
                            <span class="wt-cli-cloud-status wt-cli-cloud-status-enabled">
                                ...
                                <a  href="<?php echo admin_url('admin.php?page=wp-migration-duplicator-settings#wt-googledrive')?>"><?php echo __('Settings','wp-migration-duplicator-pro')?></a>
                            </td>
                        </tr>
                        <tr>
                            <th>
                            ...
                            <td>
                                <select class="wt-migrator-select2-single wt-migrator-cloud-import-file wt-migrator-input wt-migrator-file"  data-hidden-name="wt_mgdb_google_drive_file" style="width:100%">
                                    <option value="-1"><?php echo __('Please select a backup file','wp-migration-duplicator');?></option>
                                </select>
        ...
        {
        ?>
            <div class="wt-migrator-cloud-storage-info child-form-item child-wt_mgdb_export_option wt_mgdb_export_option_googledrive" style="display:none">
                <?php
                $url = admin_url('admin.php?page=wp-migration-duplicator-settings#wt-googledrive');
                ?>
                <span class="wt-migrator-authentication-error"><?php echo sprintf(wp_kses(__('Your account with Google drive is not authenticated please click <a href="%s">here</a> to authenticate', 'wp-migration-duplicator'), array('a' => array('href' => array(), 'target' => array()))), esc_url($url)); ?></span>
                <table class="wf-form-table" style="max-width:650px;">
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <input type="text" class="wt-migrator-input wt-migrator-file" name="wt_mgdb_google_drive_file_name" disabled placeholder="e.g. critical-backup"/>
                            </td>
                        </tr>
                    </tbody>
                </table>
